...
<div class="max-w-4xl mx-auto animate-fade-in">
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="px-6 py-3 bg-slate-900 rounded-2xl shadow-xl shadow-slate-200">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Matricule</p>
            <p class="text-xl font-black text-white italic tracking-tighter">{{ $etudiant->matricule }}</p>
        </div>
        @endif
    </div>

    <div class="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.etudiants.update', $etudiant) }}" method="POST" class="p-8 md:p-12 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                    ...
                    <h2 class="text-sm font-black text-slate-900 uppercase tracking-tighter italic">Informations Personnelles</h2>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Nom de famille</label>
                        <input type="text" name="nom" value="{{ old('nom', $etudiant->nom) }}" required 
                            class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-0 rounded-[1.5rem] font-bold text-slate-900 transition uppercase">
                        @error('nom') <p class="text-rose-500 text-[10px] font-bold ml-2">{{ $message }}</p> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Prénoms</label>
                        ...
                        @error('prenom') <p class="text-rose-500 text-[10px] font-bold ml-2">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6 pt-4">
                <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                    ...
                    <h2 class="text-sm font-black text-slate-900 uppercase tracking-tighter italic">Origine & Cursus</h2>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Série du Baccalauréat</label>
                        <input type="text" name="bac" value="{{ old('bac', $etudiant->bac) }}" placeholder="Ex: Série TI" 
                            class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-0 rounded-[1.5rem] font-bold text-slate-900 transition">
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Établissement de provenance</label>
                        <input type="text" name="provenance" value="{{ old('provenance', $etudiant->provenance) }}" placeholder="Lycée d'origine" 
                            class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-0 rounded-[1.5rem] font-bold text-slate-900 transition">
                    </div>
                </div>
            </div>
            ...
            <div class="pt-10 flex flex-col md:flex-row gap-4">
                <button type="submit" class="flex-1 py-5 bg-slate-900 text-white rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] hover:bg-indigo-600 hover:shadow-2xl hover:shadow-indigo-200 transition-all duration-300">
