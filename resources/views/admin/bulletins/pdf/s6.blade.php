...
<head>
    ...
    <style>
        ...
        .ue-row { background-color: #f1f5f9; font-weight: bold; text-align: left !important; }
...
<body>
    ...
    <div class="title">BULLETIN DE NOTES DU SEMESTRE 6</div>
    ...
    <div class="info-box">
        ...
        <strong>Classe :</strong> LP ASUR (Administration et Sécurité des Réseaux)
    </div>

    <table class="table">
        <thead>
            <tr>
                <th width="50%">DÉSIGNATION DES UNITÉS D'ENSEIGNEMENT</th>
                <th>Crédits</th>
                <th>Coeff</th>
                <th>Notes</th>
                <th>Moy. Classe</th>
        ...
        <tbody>
            @foreach($unites_enseignement as $ue)
                ...
                @foreach($ue->matieres as $matiere)
                    <tr>
                        <td style="text-align: left; padding-left: 10px;">{{ $matiere->libelle }}</td>
                        <td>{{ $matiere->credits }}</td>
                        <td>{{ $matiere->coefficient }}</td>
                        <td><strong>{{ number_format($matiere->note_finale, 2) }}</strong></td>
                        <td style="color: #666;">{{ number_format($matiere->moyenne_classe, 2) }}</td>
                    </tr>
                @endforeach
    ...
    <div class="footer">
        <div style="border: 1px solid #333; padding: 10px; width: 300px; display: inline-block;">
            <strong>RÉSULTATS DU SEMESTRE</strong><br>
            Moyenne Semestre 6 : {{ number_format($moyenne_semestre, 2) }} / 20<br>
            Crédits acquis : {{ $credits_semestre }} / 30<br>
            Résultat : <span style="text-transform: uppercase;">{{ $decision }}</span>
        </div>

        <div class="signature">
            Fait à Libreville, le {{ date('d/m/Y') }}<br><br>
            Le Directeur des Études,<br><br><br><br>
